<?php
    ...
    */
    ...
    //AWAL
     $querysearch= "SELECT id, ST_AsText(geom) as geom, st_distance_sphere(ST_GeomFromText('POINT($lng1 $lat1)'), 
     st_centroid(geom)) as jarak FROM angkot where st_distance_sphere(ST_GeomFromText(
     'POINT($lng1 $lat1)'), st_centroid(geom)) <= $rad"; 
...
//    die();
    ...
    //TUJUAN
    $querysearch="SELECT id, ST_AsText(geom) as geom, st_distance_sphere(ST_GeomFromText('POINT($lng2 $lat2)'), 
    st_centroid(geom)) as jarak FROM angkot where st_distance_sphere(ST_GeomFromText(
    'POINT($lng2 $lat2)'), st_centroid(geom)) <= $rad"; 
 ...
 //MENCARI RUTE ANGKOT YANG BERPOTONGAN DENGAN RUTE TUJUAN
    ...
    if($ketemu==false){    
        ...
        for ($i=0; $i < count($angkot_awal); $i++) { 
            ...
            $querysearch="SELECT id FROM angkot where ST_Crosses (st_geomfromtext('$geom') , geom)"; 
    ...
    if($ketemu==false){     
        ...
        for ($i=0; $i < count($angkot_awal); $i++) { 
            $geom = $geom_awal[$i];
            $querysearch="SELECT id, geom, st_distance_sphere(st_GeomFromText('$geom'), st_centroid(geom)) as jarak FROM angkot where st_distance_sphere(st_GeomFromText('$geom'), st_centroid(geom)) <= $rad"; 
            //var_dump($querysearch);
            //die(); 
            $hasil=mysqli_query($conn,$querysearch);  
            var_dump($hasil);
        ...
        for ($i=0; $i < count($angkot_awal); $i++) { 
         ...
         //   die();
            ...
            for ($q=0; ($q < count($angkot_tujuan) && $ketemu == false ); $q++) { 
                ...
                $geom2 = $geom_tujuan[$q];
                $querysearch2="SELECT id FROM angkot where ST_Crosses (st_geomfromtext('$geom2') , geom)"; 
...
//            $querysearch="SELECT id FROM angkot where ST_Crosses ('".$geom."' , geom)"; 
            $querysearch="SELECT id, geom, st_distance_sphere(st_geomfromtext('$geom', geom)) as jarak FROM angkot where st_distance_sphere(st_geomfromtext('$geom', geom)) <= $rad"; 
            $hasil=mysqli_query($conn,$querysearch);
            var_dump($hasil);
            die();
            $a=0;            
