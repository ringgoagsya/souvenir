<?php
$id=$_GET['id'];
$sql = "SELECT souvenir.id,souvenir.name as nama,souvenir.address,souvenir.cp,souvenir.employee,souvenir.owner, status.status, souvenir_type.name,ST_AsText(souvenir.geom) as geom FROM souvenir join status on souvenir.id_status=status.id join souvenir_type on souvenir.id_souvenir_type=souvenir_type.id  where souvenir.id='$id'";
