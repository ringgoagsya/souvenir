<?php
$id=$_GET['id'];
$sql = "SELECT small_industry.id,small_industry.name as nama,small_industry.address,small_industry.cp,small_industry.employee,small_industry.owner, status.status, industry_type.name,ST_AsText(small_industry.geom) as geom FROM small_industry join status on small_industry.id_status=status.id join industry_type on small_industry.id_industry_type=industry_type.id  where small_industry.id='$id'";
						...
						<label for="jns">Tipe Industri</label>
						     <select required name="jns" id="jns" class="form-control">
						         <?php
					              ...
					              $jns="select * from industry_type ";
					              $hasil=mysqli_query($conn,$jns);
					              while($dt = mysqli_fetch_array($hasil)){
